...
class Game {   
	...
	extraPointImage: HTMLImageElement;
	pausa: boolean = false;
	fps: any;
	levelPerPoints: number;
	...
	extraPoints: any;
	limite: {superior: number, inferior:number, izquierdo: number, derecho: number};
	...
	draw ():void {
		...
		this.extraPoints.forEach(element => {
			this.canvas.ctx.clearRect( this.canvas.paddingLateral, element.top, this.canvas.bordes, element.borders * 8 );
			for (i=0;i<element.borders;i++) {
				this.canvas.ctx.drawImage( this.extraPointImage, this.canvas.paddingLateral, element.top + i * 8 );
			}
			this.canvas.ctx.rotate(Math.PI / -2);
			this.canvas.ctx.fillText( element.points, -1*(element.top + i*4 + 18), this.canvas.paddingLateral - 15 );
			this.canvas.ctx.rotate(Math.PI / 2);
		});
		...
		for (let i=0;i<this.lives;i++) {
			this.canvas.ctx.drawImage( this.image, center - (this.lives * 35 / 2) + 35*i, 125, 30, 30 );
		}
	...
	colisionX(derecha: boolean): void {
		this.rebote.start();
	...
	liveLost():void {
		...
		if ( this.lives ) {
			...
			setTimeout( () => {
				this.pausa = false;
				this.pelota.top 	= this._toPixels("50%", "vertical") - 8;
				this.pelota.left 	= this._toPixels("50%", "horizontal") - 8;
				this.startCountDown();
			}, 2000 );
		}
	}
	...
	_countDown() {
		...
		if (this.countDownSteps) {
			...
			this.countDownSteps--;
			setTimeout(()=>{ this._countDown() }, 5000 / 3);
		} else {
			this.animation();
		}
	}
	...
	// ------------- METODOS PRIVADOS ------------- //
	private _toPixels ( value:string, o:dimentionType ) {
		if (!/^([0-9]{1,2}|100)%$/.test( value )) throw "_toPixels: invalid longitude.";
		let a = o==="horizontal"? this.limite.derecho - this.limite.izquierdo: this.limite.inferior - this.limite.superior,
			b = o==="horizontal"? this.limite.izquierdo: this.limite.superior;
...
type dimentionType = "horizontal" | "vertical";
type longitude = string | number;
let game;
